...
public class ExampleFarmingBot : BotBase
{
    public override async Task RunAsync()
    {
        ...
        while (!CancellationToken.IsCancellationRequested)
        {
            ...
            // 1. Find Enemy
            if (await Vision.WaitForImageAsync("enemy_icon.png", timeoutMs: 2000))
            {
                Log("Enemy found! Attacking...");
                
                // 2. Attack
                if (await Vision.ClickImageAsync("enemy_icon.png"))
                {
                    // Wait for attack animation
                    await Delay(1500); 
                    
                    // 3. Collect Loot if available
                    if (await Vision.ExistsAsync("loot_box.png"))
                    {
                        Log("Loot found! Collecting...");
                        await Vision.ClickImageAsync("loot_box.png");
                        await Delay(500);
